...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = withMaterials
      ? `Ты опытный сметчик в России. Проанализируй этот строительный чертёж или фото объекта. Составь подробную смету в рублях по актуальным рыночным ценам 2025-2026 года для региона ${region}. Для каждой позиции подбери конкретные материалы с указанием марки, производителя и артикула где возможно. Включи стоимость материалов в смету отдельными строками. Верни ТОЛЬКО валидный JSON без markdown и комментариев: {"summary":"краткое описание объекта","items":[{"name":"наименование","unit":"ед.","qty":0,"price":0,"total":0}],"total_rub":0,"notes":"замечания"}`
      : `Ты опытный сметчик в России. Проанализируй этот строительный чертёж или фото объекта. Составь подробную смету в рублях по актуальным рыночным ценам 2025-2026 года для региона ${region}. Верни ТОЛЬКО валидный JSON без markdown и комментариев: {"summary":"краткое описание объекта","items":[{"name":"наименование","unit":"ед.","qty":0,"price":0,"total":0}],"total_rub":0,"notes":"замечания"}`
